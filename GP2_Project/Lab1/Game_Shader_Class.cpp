...
void Game_Shader_Class::init(const std::string& filename)
{
	...
	shaders[0] = CreateShader(LoadShader(filename + ".vert"), GL_VERTEX_SHADER); // create vertex shader using input name
	shaders[1] = CreateShader(LoadShader(filename + ".frag"), GL_FRAGMENT_SHADER); // create fragment shader using input name
